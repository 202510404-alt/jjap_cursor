...
# 🪓 3대 실무 부서 영입!
from .patch_parser import LLMPatchParser
from .diff_viewer import DiffViewer
from .file_writer import AtomicFileWriter
...
def run_jjap_pipeline(
    ...
    try:
        ...
        sync_project_index(file_scanner)
        ...
        # [Step 5] Single Parsing Gate 통과 (전담 부서 사용)
        print("🛡️ [Step 5] 단일 파싱 검증 게이트 통과 중...")
        transaction: PatchTransaction = patch_parser.parse(raw_patch_response)

        # [Step 6] Unified Diff Preview (디자이너 부서 사용)
        print("\n🔍 [Step 6] === UNIFIED DIFF PREVIEW ===")
        diff_viewer.render(transaction)
        ...
        # [Step 8] Transaction Apply & Atomic Write (원자적 쓰기 부서 사용)
        print("\n💾 [Step 8] 무결성 수술실 입장: OS 레벨 원자적 쓰기 및 트랜잭션 반영 시작...")
        write_result = file_writer.apply(transaction)
